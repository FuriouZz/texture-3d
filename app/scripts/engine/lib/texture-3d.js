 ...
 */
function Texture3D( gl, format, type, internal ){
  this._uid = _UID++;
  this.gl = gl;
...
Texture3D.prototype = {
  ...
  fromImage : function( img ){
    ...
    gl.bindTexture( T3D, this.id );
    gl.texImage3D(  T3D, 0, this.internal, this.width, this.height, 1, 0, this.format, this.type, img );
  },
   ...
   */
  fromData : function( width, height, depth, data ){
    var gl = this.gl;
    ...
    gl.bindTexture( T3D, this.id );
    gl.texImage3D( T3D, 0, this.internal, width, height, depth, 0, this.format, this.type, data );
  },
  ...
  setFilter : function( smooth, mipmap, miplinear ){
    ...
    gl.texParameteri( T3D, gl.TEXTURE_MAG_FILTER, getFilter( !!smooth, false, false ) );
    gl.texParameteri( T3D, gl.TEXTURE_MIN_FILTER, filter );
  },

  setLevel : function( level ){
    var gl = this.gl
    gl.texParameteri(gl.TEXTURE_3D, gl.TEXTURE_BASE_LEVEL, 0)
    gl.texParameteri(gl.TEXTURE_3D, gl.TEXTURE_MAX_LEVEL , level)
  },
  ...
  wrap : function( wrap ) {
    var gl = this.gl;
    gl.texParameteri( T3D, gl.TEXTURE_WRAP_S, wrap );
    gl.texParameteri( T3D, gl.TEXTURE_WRAP_T, wrap );
  }
